...
- `AGENTS.md` (this file) - General domain knowledge
...
> ⚠️ **MUST REBUILD THE FRONTEND BUNDLE FOR CLIENT CHANGES TO TAKE EFFECT IN PROD.**
> The server serves the committed `frontend_dist/` bundle, NOT `frontend/src/`. Editing
> and committing source under `frontend/src/` does nothing in prod until the bundle is
> regenerated. After ANY frontend change:
> ```bash
> cd frontend && npm run deploy   # vite build + copy dist/* → ../frontend_dist/
> ```
> Then commit the updated `frontend_dist/` and push so Coolify deploys it. A frontend
> source commit without a matching `frontend_dist/` rebuild = change not live.
...
- Stage only relevant files (exclude `.Codex/settings.local.json`)
...
- **`AGENTS.md`** - Sub-agent workflow, domain model, general guidelines
